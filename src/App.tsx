...
function BackupControls({ t, data, theme, locale, setData, download }: any) {
  ...
  const exportCloud = async () => { const now = new Date(); const pad = (x: number) => String(x).padStart(2, "0"); const name = `careerflow-backup-${now.getFullYear()}-${pad(now.getMonth() + 1)}-${pad(now.getDate())}-${pad(now.getHours())}${pad(now.getMinutes())}.json`; const backupFile = new File([JSON.stringify({ ...snapshot(), exportedAt: Date.now() }, null, 2)], name, { type: "application/json" }); try { if (navigator.share && navigator.canShare?.({ files: [backupFile] })) await navigator.share({ files: [backupFile], title: "CareerFlow Backup" }); else download(name, await backupFile.text(), "application/json"); const timestamp = Date.now(); localStorage.setItem("careerflow-last-export", String(timestamp)); setLastExport(timestamp); } catch (e) { if ((e as DOMException).name !== "AbortError") setError(locale === "ja" ? "書き出しに失敗しました" : locale === "en" ? "Export failed" : "导出失败"); } };
