...
function BackupControls({ data, theme, locale, setData }: any) {
  ...
  const labels = locale === "ja" ? { exportBackup:"バックアップを書き出す", restoreFile:"バックアップを復元", permission:"ファイルアプリで保存先を選択できます。", last:"前回の書き出し", never:"まだバックアップを書き出していません" } : { exportBackup:"导出备份", restoreFile:"恢复备份", permission:"请在文件 App 中选择保存位置。", last:"上次导出", never:"尚未导出备份" };
