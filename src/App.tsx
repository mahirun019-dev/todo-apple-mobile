...
function SettingsPanel({ t, theme, setTheme, locale, setLocale, close, data, setData, iconRef, csv, json, upload, importCsv, importJson, download }: any) {
  ...
  const tabs = [["general", "一般"], ["appearance", "表示"], ["language", "言語"], ["data", "データとバックアップ"], ["about", "CareerFlowについて"]];
  return <SettingsDrawer title={t.settings} close={close}><div className="desktop-settings-layout"><nav className="desktop-settings-nav">{tabs.map(([key, text]) => <button className={tab === key ? "active" : ""} onClick={() => setTab(key)} key={key}>{text}</button>)}</nav><div className="desktop-settings-content">
    {tab === "general" && <section className="settings-section"><h3>このデバイスの保存状況</h3><div className="settings-stats">{[["企業数", data.companies.length], ["日程数", data.events.length], ["資料数", data.materials.length], ["面接記録数", data.interviews.length], ["準備事項数", data.preparations.length], ["データベースバージョン", "v" + data.schemaVersion]].map(([label, value]) => <div key={String(label)}><span>{label}</span><strong>{value}</strong></div>)}</div></section>}
    {tab === "appearance" && <section className="settings-section"><h3>表示</h3><div className="settings-segmented">{(["light", "dark", "system"] as Theme[]).map((x) => <button className={theme === x ? "active" : ""} onClick={() => setTheme(x)} key={x}>{t[x]}</button>)}</div></section>}
    {tab === "language" && <section className="settings-section"><h3>言語</h3><div className="settings-segmented">{(["zh", "ja", "en"] as Locale[]).map((x) => <button className={locale === x ? "active" : ""} onClick={() => setLocale(x)} key={x}>{x === "zh" ? "中文" : x === "ja" ? "日本語" : "English"}</button>)}</div></section>}
    {tab === "data" && <section className="settings-section settings-data-section"><h3>CSV</h3><div className="settings-actions"><button onClick={() => csv.current?.click()}><Upload />CSV読込</button><button onClick={() => download("careerflow-materials.csv", Papa.unparse(data.materials), "text/csv")}><Download />CSV書出</button></div><h3>バックアップ</h3><BackupControls t={t} data={data} theme={theme} locale={locale} setData={setData} download={download} /><div className="settings-actions"><button onClick={() => json.current?.click()}><RotateCcw />バックアップファイルから復元</button><button onClick={() => download("careerflow-backup.json", JSON.stringify(data, null, 2), "application/json")}><Archive />完全バックアップをダウンロード</button></div><div className="settings-danger"><h3>危険区域</h3><button className="danger-button" onClick={() => { if (window.confirm("サンプルデータを削除しますか？")) setData((d: Data) => ({ ...d, companies: d.companies.filter((x) => !x.tags.includes("sample") && !demoNames.includes(x.name)), materials: d.materials.filter((x) => !x.tags.includes("sample")) })); }}><Trash2 />サンプルデータを削除</button></div></section>}
    {tab === "about" && <section className="settings-section"><h3>CareerFlowについて</h3><div className="settings-about-list"><p>CareerFlow バージョン 1.0</p><p>データベースバージョン v{data.schemaVersion}</p><p>PWA ステータス: standalone 対応</p><p>アイコン: CareerFlow ブランドアイコン</p><p>プライバシー: データは主にこのデバイスに保存されます。</p><p>オープンソースライセンス: MIT License</p></div></section>}
